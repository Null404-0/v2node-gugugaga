...
- 安装 / 管理脚本的**自更新地址改为指向本仓库**（`Null404-0/v2node-gugugaga` 的 `main` 分支），避免 `v2node update` 时被上游脚本覆盖。
...
- **首个独立 Release 已发布**：`v0.4.1` 已产出 `v2node-linux-64.zip`、`v2node-linux-arm64-v8a.zip`、`v2node-linux-s390x.zip`，安装脚本的二进制版本检测与下载地址已从上游切换为本仓库 release。
